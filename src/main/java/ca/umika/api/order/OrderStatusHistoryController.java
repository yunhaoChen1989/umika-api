...
public class OrderStatusHistoryController {
    ...
    @GetMapping("/{id}")
    public OrderStatusHistoryDto findById(@PathVariable UUID id) {
    ...
    public ResponseEntity<OrderStatusHistoryDto> create(@RequestBody OrderStatusHistoryDto dto) {
        ...
        return ResponseEntity.created(URI.create("/api/v1/order-status-history/" + created.id())).body(created);
    }

    @PutMapping("/{id}")
    public OrderStatusHistoryDto update(@PathVariable UUID id, @RequestBody OrderStatusHistoryDto dto) {
